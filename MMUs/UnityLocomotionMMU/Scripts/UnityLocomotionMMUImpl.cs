...
namespace UnityLocomotionMMU
{
    ...
    public class UnityLocomotionMMUImpl : UnityMMUBase
    {
        ...
        public override MBoolResponse AssignInstruction(MInstruction instruction, MSimulationState simulationState)
        {
            ...
            this.ExecuteOnMainThread(() =>
            {
                ...
                if (this.trajectory == null || this.trajectory.Poses.Count == 0)
                {
                    ...
                    MMICSharp.Logger.Log(MMICSharp.Log_level.L_ERROR, $"No path found in assign instruction. Aborting MMU.");
        ...
        private MotionTrajectory2D ComputePath(Vector2 from, Vector2 target, bool filterSceneObjects = true)
        {
                ...
                try
                {
                    ...
                    if (filterSceneObjects)
                    {
                        ...
                        for (int i = sceneObjects.Count - 1; i >= 0; i--)
                        {
                            ...
                            {
                                MMICSharp.Logger.Log(MMICSharp.Log_level.L_DEBUG, $"Removing scene object {sceneObjects[i].Name}, that is not included in path planning.");
            ...
            MMICSharp.Logger.Log(MMICSharp.Log_level.L_DEBUG, "Computed path elements: " + computedPath.Count);
        ...
        private bool OrientateTowards(Vector2 position, TimeSpan timespan, float threshold)
        {
            ...
            float currentDeltaAngle = Vector3.SignedAngle(currentDirection, new Vector3(targetDirection.x, 0, targetDirection.y), Vector3.up);
            ...
            if (Math.Abs(currentDeltaAngle) > threshold)
            {
                ...
                float goalDelta = currentDeltaAngle;
                float delta = Math.Clamp(goalDelta, -maxAngle, +maxAngle);
                ...
                float animationDirectionValue = (delta / maxAngle) / 2.0f;
