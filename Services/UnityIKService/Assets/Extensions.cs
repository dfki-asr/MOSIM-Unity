...
namespace UnityIKService
{
    ...
    #region HelperFunctions
        ...
        public static void ApplyFBBIK(Dictionary<MJointType, GameObject> jointTypeToGameObject, FullBodyBipedIK finalBPIK, MVector3 targetPosition, MQuaternion targetRotation , MJointType jointType, float posWeight = 1f, float rotWeight = 1f)
        {
            ...
            if(fbbikEffector != FullBodyBipedEffector.Body)
            {
                ...
                ikEffector.positionWeight = posWeight;
                ikEffector.rotationWeight = rotWeight;
                if(jointType == MJointType.LeftWrist)
                {
                    finalBPIK.solver.leftArmMapping.weight = 1;
                } else if(jointType == MJointType.RightWrist)
                {
                    finalBPIK.solver.rightArmMapping.weight = 1;
                }
